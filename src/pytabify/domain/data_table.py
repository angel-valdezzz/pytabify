...
class DataTable:
    ...
    def __init__(self, rows: list[DTRow] | None = None, schema: list[str] | None = None):
        self._schema = [str(name) for name in (schema or [])]
        self._rows = rows or []
        self._sync_rows()
    ...
    def set_value(self, row_index: int, column_name: str, value: Any):
